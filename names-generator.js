 ...
 * formatted as "adjective_surname". For example `focused_turing`. If retry is set, a random
 * integer between 0 and 10 will be added to the end of the name, e.g `focused_turing3`.
 * @param {*} [retry] Any value to indicate a retry.
 * @return {string} A random name formatted as "adjective_surname".
 */
function getRandomName(retry) {
  let name =
    `${adjectives[_getRandomInt(adjectives.length)]}_${surnames[_getRandomInt(surnames.length)]}`;

  if (name === 'boring_wozniak') /* Steve Wozniak is not boring */ {
    name = getRandomName(retry);
  } else if (retry) {
    name += _getRandomInt(retry);
  }
